...
export default function ExamCenterManagement() {
  ...
  const fetchExamCenters = async () => {
    try {
      ...
      setError('Erreur lors du chargement des centres d\'examen')
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await client.put(`/manager/exam-centers/${editingId}`, dataToSend)
        setSuccess('Centre d\'examen modifié avec succès!')
      } else {
        await client.post('/manager/exam-centers', dataToSend)
        setSuccess('Centre d\'examen créé avec succès!')
      }
      setFormData({
        name: '',
        address: '',
  ...
  const handleDelete = async (centerId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce centre d\'examen?')) {
      try {
        await client.delete(`/manager/exam-centers/${centerId}`)
        setSuccess('Centre d\'examen supprimé!')
        fetchExamCenters()
      } catch (err) {
  ...
  return (
    <div className="user-management">
      <header className="management-header">
        <h1>Gestion des Centres d'Examen</h1>
        <p>Gérez tous vos centres d'examen</p>
      </header>
      ...
      <div className="management-content">
        ...
        <div className="centers-list">
          {getFilteredExamCenters().length === 0 ? (
            <p className="no-results">Aucun centre d'examen trouvé</p>
          ) : (
            <table className="data-table">
              ...
              <tbody>
                {getFilteredExamCenters().map((center) => (
                  <tr key={center.id}>
                    <td>{center.name}</td>
                    <td>{center.city}</td>
