...
export default function AdminManagement() {
  ...
  const handleToggleStatus = async (adminId) => {
    try {
      await axios.put(`http://localhost:8000/api/admin/users/${adminId}/status`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const handleDelete = async (adminId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer cet admin?')) {
      try {
        await axios.delete(`http://localhost:8000/api/admin/users/${adminId}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        setSuccess('Admin supprimé!')
        fetchAdmins()
      } catch (err) {
    ...
    <div className="user-management">
      <header className="management-header">
        <div>
          ...
          <p>Créez et gérez les administrateurs du système</p>
        </div>
